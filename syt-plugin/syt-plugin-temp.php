...
function syt_sig_authorise_func($entry, $form) { 
	...
	if((int)$form['id'] > 6 && (int)$form['id'] < 22)
	{
		...
		if (current_user_can('edit_post',$entry["4"]) && !$_SESSION['SYT_READ_ONLY_USER']) 
		{
...
function check_custom_authentication ($username) {
	...
	$_SESSION['SYT_READ_ONLY_USER'] = false;
...
function syt_read_only_func( $atts, $content = null ) {
	...
	if($a['ifuseris'] == 'demo' && $_SESSION['SYT_DEMO_USER']  || $a['ifuseris'] == 'readOnly' && $_SESSION['SYT_READ_ONLY_USER'] || $a['ifuseris'] == 'readWrite' && !$_SESSION['SYT_READ_ONLY_USER'])
	{
		return $content;
	}
	else
...
function syt_show_dropdown_options_func( $atts, $content = null ) {
	...
	<optgroup label = "Implementation + Operations">
		...
		<option disabled="disabled">-- Management \ Board</option>
		<option disabled="disabled">-- Chief Exec.</option>
	...
	<optgroup label = "Checking">
		...
		<option disabled="disabled">Incident Investigation</option>
	...
	<optgroup label = "Topic Specific Polies ">
		<option disabled="disabled">Contents</option>
		<option disabled="disabled">Accidents + First-aid</option>
		...
		<option disabled="disabled">Fire Safety</option>
		...
		<option disabled="disabled">Water Systems – Legionella</option>
...
function syt_show_dropdown_downloads_func( $atts, $content = null ) {
	if($_SESSION['SYT_DEMO_USER'])
	{
		...
		<option  value="#" disabled="disabled">H&amp;S Law Leaflet from HSE</option>
		...
		<option  value="#" disabled="disabled">Vehicle Check Template</option>
		<option  value="#" disabled="disabled">Workplace Inspection Record Template</option></optgroup>
		...
		<optgroup label = "Risk Assessment Models and Templates"><option  value="#" disabled="disabled">DSE Risk Assessment Template</option>
	...
	else
	{
		...
		<optgroup label = "Risk Assessment Models and Templates"><option value="http://www.safetyyoutrust.com/wp content/uploads/2014/09/DSE Risk Assessment Template.docx">DSE Risk Assessment</option>
