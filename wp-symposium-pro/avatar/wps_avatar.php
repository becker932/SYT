...
function user_avatar_add_photo_step1($uid)
{
	...
	<form enctype="multipart/form-data" id="uploadForm" method="POST" action="<?php echo admin_url('admin-ajax.php'); ?>?action=user_avatar_add_photo&step=2&uid=<?php echo $uid; ?>" >
		...
		<p class="submit"><input type="submit" class="wps_submit btn btn-primary" value="<?php esc_attr_e('Upload'); ?>" /></p>
	</form>
	</div>
...
function user_avatar_add_photo_step2($uid)
{
		...
		<input type="submit" class="wps_submit btn btn-primary" style="margin-left:0;" id="user-avatar-crop-button" value="<?php esc_attr_e('Crop Image', WPS2_TEXT_DOMAIN); ?>" />
		</div>
		</form>
...
function user_avatar_add_photo_step3($uid)
{
	...
	$cropped_full = user_avatar_core_avatar_upload_path($uid).time()."-wpsfull.jpg";
	$cropped_thumb = user_avatar_core_avatar_upload_path($uid).time()."-wpsthumb.jpg";
...
function user_avatar_get_avatar($id,$width) {
	...
	if ($id):
		...
		return '<img src="'.plugins_url('/wp-symposium-pro/avatar/images/mystery-man.jpg').'" width="'.$width.'" height="'.$width.'" class="avatar" />';;
