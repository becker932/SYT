...
*/
			...
			if(!is_user_logged_in()) // The User/Member is NOT logged in.
			{
				...
				echo '</form>'."\n";*/
echo '<div id="login-box" class="module module-callout ui button-right light-green  rounded gradient ">'."\n";
echo '<div class="logged-out">'."\n";
echo '<form method="post" action="'.esc_attr(site_url('wp-login.php', 'login_post')).'" class="ws-plugin--s2member-pro-login-widget-form">'."\n";
echo '<table class="syt-login-widget">'."\n";
echo '<tr class="syt-login-widget"><td class="syt-login-widget-td"><b>Login</h3></b>'."\n";
echo '<td align="right" class="syt-login-widget-td">'."\n";
$reg_cookies_ok_url = (is_multisite() && c_ws_plugin__s2member_utils_conds::is_multisite_farm() && is_main_site()) ? c_ws_plugin__s2member_utils_urls::wp_signup_url() : c_ws_plugin__s2member_utils_urls::wp_register_url();
echo ($options['signup_url']) ? '<a href="'.esc_attr(($options['signup_url'] !== '%%automatic%%') ? $options['signup_url'] : ((c_ws_plugin__s2member_register_access::reg_cookies_ok()) ? $reg_cookies_ok_url : $links['membership_options_page'])).'" tabindex="-1">'._x('Register', 's2member-front', 's2member').'</a> ' : '';
echo '</td'."\n";
echo '</tr><tr class="syt-login-widget"><td  class="syt-login-widget" colspan="2">'."\n";			
echo '<div class="field field-username">'."\n";
echo '<div class="prefix">'."\n";
echo '<div class="sprite-user"><img src="/wp-content/uploads/2014/09/userSprite.png"></div>'."\n";
echo '</div>'."\n";
echo '<input type="text" name="log" id="ws-plugin--s2member-pro-login-widget-username" class="syt-formControl" title="Username" />'."\n";
echo '</div></td></tr><tr class="syt-login-widget"><td  class="syt-login-widget" colspan="2">'."\n";				
echo '<div class="field field-password">'."\n";
echo '<div class="prefix">'."\n";
echo '<div class="sprite-password"><img src="/wp-content/uploads/2014/09/pwordSprite.png"></div>'."\n";
echo '</div>'."\n";
echo '<input type="password" name="pwd" id="ws-plugin--s2member-pro-login-widget-password" class="syt-formControl" title="Password" />'."\n";
echo '<div class="postfix">'."\n";
	if($redirect_to) echo '<input type="hidden" name="redirect_to" value="'.esc_attr($redirect_to).'" />'."\n".
	(empty($seeking) ? '<input type="hidden" name="redirect_to_automatic" value="1" />'."\n" : '');
echo '<input type="submit" class="syt-login-btn" value="'.esc_attr(_x('', 's2member-front', 's2member')).'" />'."\n";
echo '<div class="sprite-submit"><img src="/wp-content/uploads/2014/09/loginBtnW.png"></div>'."\n";
echo '</div>'."\n";
echo '</div>'."\n";
echo '</td></tr>'."\n";
echo '<tr class="syt-login-widget"><td class="syt-login-widget-td"><label><input type="checkbox" name="rememberme" value="forever" />'._x('Remember Me', 's2member-front', 's2member').'</label></div>'."\n";			
echo '</td><td align="right"  class="syt-login-widget-td"><a href="'.esc_attr(wp_lostpassword_url()).'" tabindex="-1">'._x('Forgot Password?', 's2member-front', 's2member').'</a>'."\n";				
echo '</td></tr></table>'."\n";
...
echo '</div>'."\n";
			...
			else if(is_user_logged_in() && is_object($user = wp_get_current_user()) && !empty($user->ID) && ($user_id = $user->ID))
			{
				...
				echo ($options['display_name']) ? '<div class="ws-plugin--s2member-pro-login-widget-profile-summary-name">Welcome '.esc_html($user->display_name).'</div>'."\n" : '';
