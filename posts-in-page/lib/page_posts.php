...
class ICPagePosts {
	...
	public function output_posts() {
		error_log("output the post ".$this->args);
		...
		{
			$myId = $user_ID;
			//error_log("reckons it doesn't hasParent".$hasParent.">>".($user_ID).">>".$user_id);
				...
				if ( in_category('customisable', $page_posts->the_post.get_the_ID()) ) {
					...
					else
					{
						...
						if ( $my_query->have_posts() ){
							//echo 'returned a version of the doc with the right value in the custom field'.count($my_query);  
						     //it exists
							...
							if(get_post_meta( $my_query->posts[0]->ID, 'syt-draft-mode', true)=='true')
							{
								if($_SESSION['SYT_READ_ONLY_USER'])
								{
									...
									error_log("TRYING TO GET A VALID REVISION NOT A DRAFT ".$my_query->posts[0]->ID);
									$args = array(
										'orderby' => 'date',
										'order'   => 'DESC',
									);
						...
						else
						{
							...
							else
							{
								...
								$content = "<div class='raptor-editable-post' data-post_id='{$post_id}'>{$content}</div>";
								$output .= $content;
								$output .= $customCount;
								break;
		...
		{
			$output = str_replace( "class='raptor-editable-post'", "class='raptor-editable-post' style='background-color:peachpuff'", $output );
		}
		...
		if( count(get_post_meta( $SYT_custom_query, 'syt_sig')) >0)
		{
			...
			foreach ( $my_custom_field as $key => $value ) 
			{
				...
				else
				{
					...
					if($_SESSION['SYT_READ_ONLY_USER'] ) //|| $a[0] !="")
					{
			...
			if(empty($_SESSION['SYT_READ_ONLY_USER']))
			{
				if(get_post_meta( $SYT_custom_query, 'syt-draft-mode', true) != "true")
				{
