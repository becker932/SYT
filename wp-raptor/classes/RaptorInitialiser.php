...
class RaptorInitialiser {
    ...
    public function initialise() {
        ...
        error_log("RaptorInitialiser::adminViewingPosts()".(!empty($_SESSION['SYT_DEMO_USER'])));
        if (RaptorStates::adminViewingPosts() && $this->options->allowInPlaceEditing() ||  !empty($_SESSION['SYT_DEMO_USER']) &&  $this->options->allowInPlaceEditing()) {
            error_log("RaptorInitialiser:: in if statement");
            if(!empty($_SESSION['SYT_READ_ONLY_USER']))return;
            error_log("RaptorInitialiser:: SYT_READ_ONLY_USER exists");
            ...
            error_log("RaptorInitialiser:: endif exists");
        }
        error_log("RaptorInitialiser:: outside if");
    }
}